...
class NouveauDocumentView(QWidget):
    ...
    def setup_table(self):
        columns = [
            ...
            DocumentLineColumn("designation", "DESIGNATION", width=220, min_width=120),
            DocumentLineColumn(
                "quantite",
                "QTY",
                width=72,
        ...
        self.DocumentLinesWidget.set_column_updates(
            ...
            updates={
                ...
                "montant_net_ht": lambda row: round(
                    quantity(row) * unit_net_ht(row), 6
                ),
                "montant_net_ttc": lambda row: round(
                    quantity(row) * unit_net_ttc(row), 6
                ),
                "montant_remise": lambda row: round(
                    quantity(row) * unit_discount(row), 6
                ),
